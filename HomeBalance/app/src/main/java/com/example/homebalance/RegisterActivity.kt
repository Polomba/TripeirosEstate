        ...
        call.enqueue(object : Callback<Any> {
            override fun onResponse(call: Call<Any>, response: Response<Any>) {
                if (response.isSuccessful) {
                    Toast.makeText(this@RegisterActivity, "Utilizador criado com sucesso!", Toast.LENGTH_SHORT).show()
                    val intent = Intent(this@RegisterActivity, LoginActivity::class.java)
                    startActivity(intent)
                } else {
                    Toast.makeText(this@RegisterActivity, "Email já em utilização ou password impossivel!", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onFailure(call: Call<Any>, t: Throwable) {
                print("")
                Toast.makeText(this@RegisterActivity, "Erro na requisição: " + t.message, Toast.LENGTH_SHORT).show()
            }
        })
    }
}